...
class TestNotifier extends Notifier {
  protected async notifyOnCommunityPointsEarned(
    data: CommunityPointsUserV1_PointsEarned,
    channelLogin: string,
  ): Promise<void> {
    return;
  }

  protected async notifyOnDropClaimed(
    drop: TimeBasedDrop,
  ...
  protected async notifyOnDropProgress(
    drop: TimeBasedDrop,
    campaign: DropCampaign,
  ): Promise<void> {
    return;
  ...
  protected async notifyOnDropReadyToClaim(
    drop: TimeBasedDrop,
    campaign: DropCampaign,
  ): Promise<void> {
    return;
  ...
  protected async notifyOnNewDropCampaign(
    campaign: DropCampaign,
  ): Promise<void> {
    return;
  }
...
function getCommunityPointsEarnedData(options?: {
  reasonCode: CommunityPointsUserV1_PointsEarned_ReasonCode;
}): CommunityPointsUserV1_PointsEarned {
  const getRandomReasonCode =
    (): CommunityPointsUserV1_PointsEarned_ReasonCode => {
      const reasonCodes: CommunityPointsUserV1_PointsEarned_ReasonCode[] = [
        "WATCH",
        "CLAIM",
        "WATCH_STREAK",
        "RAID",
...
describe("notifyOnCommunityPointsEarned", () => {
  ...
  test("filterReasons", async () => {
    const reasons: Event_CommunityPointsEarned_ClaimReason[] = [
      "watch",
      "claim",
      "watch_streak",
      "raid",
    ...
    for (let i = 0; i < reasons.length; ++i) {
      ...
      const spy = jest.spyOn<TestNotifier, any>(
        notifier,
        "notifyOnCommunityPointsEarned",
      );

      const data1 = getCommunityPointsEarnedData({
        reasonCode: reasonCodes[i],
      });
      await notifier.onCommunityPointsEarned(data1, "channel_login_1");

      const data2 = getCommunityPointsEarnedData({
        reasonCode: reasonCodes[(i + 1) % reasonCodes.length],
      });
...
describe("RateLimitedNotifier", () => {
  test(
    ...
    async () => {
      ...
      const mockNotifierSendNotification = jest
        .spyOn<TestRateLimitedNotifier, any>(notifier, "sendNotification")
        .mockImplementation(() => {
          // Remove old requests
            ...
            throw new AxiosError(
              ...
              undefined,
              {
                data: (
                  ...
                  (new Date().getTime() - sentRequests[0].getTime()) / 1000
                ).toString(),
                status: 429,
      ...
      for (let i = 0; i < 10; ++i) {
        await notifier.onCommunityPointsEarned(
          getCommunityPointsEarnedData(),
          "channel_login",
        );
      }
